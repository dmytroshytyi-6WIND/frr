					 ...
					 uint32_t sid_wide_func)
{
	...
	if (format->type == ZEBRA_SRV6_SID_FORMAT_TYPE_COMPRESSED_USID) {
		...
		if ((sid_func >= elib_start) && (sid_func <= elib_end)) {
		...
		} else if ((sid_func >= ewlib_start) && (sid_func <= ewlib_end)) {
			 ...
			 */
			sid_wide_func_ptr =
				zebra_srv6_sid_func_alloc(sid_wide_func);
			listnode_add(block->u.usid.wide_lib[sid_func]
					     .func_allocated,
				     sid_wide_func_ptr);
			block->u.usid.wide_lib[sid_func].num_func_allocated++;
		} else {
					...
					uint32_t *sid_func)
{
	...
	if (format->type == ZEBRA_SRV6_SID_FORMAT_TYPE_COMPRESSED_USID) {
		...
		if (listcount(block->u.usid.lib.func_released) != 0) {
			 ...
			 */
			sid_func_ptr =
				listnode_head(block->u.usid.lib.func_released);
			*sid_func = *sid_func_ptr;
			 ...
			 */
			*sid_func = block->u.usid.lib.first_available_func;
			block->u.usid.lib.first_available_func++;
		}
		...
		/* Check if we ran out of available SID functions */
		if (block->u.uncompressed.first_available_func > dynamic_end) {
...
int release_srv6_sid(struct zserv *client, struct zebra_srv6_sid_ctx *zctx)
{
	...
	if (listcount(zctx->sid->client_list) == 0) {
		...
		if (!(zctx->sid->block->sid_format->type ==
			      ZEBRA_SRV6_SID_FORMAT_TYPE_COMPRESSED_USID &&
